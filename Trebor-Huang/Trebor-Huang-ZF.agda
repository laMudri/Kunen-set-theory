...
module Trebor-Huang.Trebor-Huang-ZF where
...
⋃∅≡∅ = Extensional
    \ _ -> false-≡
    \ { (exists _ ()) } -- \ { (exists bot [ lol∈bot , bot∈∅ ] ) -> bot∈∅ }
...
⋃⟦x⟧ {x = x} = Extensional
    \ z -> equiv-equal
    [ (\ { (exists x [ z∈x , refl𝕍 ]) -> z∈x }) , (\ z∈x -> exists x [ z∈x , refl𝕍 ]) ] -- changed the implicit _ to explicit x
...
module Pairing where
    abstract
        ...
        x∈⟨x,y⟩ : x ∈ ⟨ x , y ⟩
        x∈⟨x,y⟩ = exists ∅ [ (λ ()) , ≡-≗ Pair-∅ ] -- absurd pattern suffices
...
private
    ...
    Pair-equal-right-alternate : ∀ x y z w -> ⟨ x , y ⟩ ≡ ⟨ z , w ⟩ -> (y ≗ z) ∨ (y ≗ w)
    Pair-equal-right-alternate x y z w eq 
        = equal-equiv (Extensional-converse eq y) (ι₂ refl𝕍)
    ...
    abstract
        ...
        private
            Pairwise-Intersection-> : (x y z : 𝕍)
                -> z ∈ (x ∩ y) -> (z ∈ x) ∧ (z ∈ y)
            Pairwise-Intersection-> x y z [ _ , cap ] =
                [ cap x (ι₁ refl𝕍) , cap y (ι₂ refl𝕍) ]
...
count-ord n i = injective-ord (count (ord n) i) n (ord-count (ord n) i)
